...
df <- read_csv(
  '/Users/beiniou/Desktop/SNQ Pipeline/Social+Network+Questionnaire+-+Bethany+working_October+28,+2025_23.51.csv',
...
ego_level_network_summary <- df %>%
  mutate(
    # --- Merge childcare "other" text where present (robust to missing column) ---
    childcare_type = {
      # If the free-text column does not exist, treat as all-NA
      other_exists <- "childcare_type_8_TEXT" %in% names(.)
      other_text <- if (other_exists) childcare_type_8_TEXT else NA_character_
      other_text <- ifelse(is.na(other_text) | trimws(other_text) == "", NA_character_, trimws(other_text))
      base_val <- ifelse(is.na(childcare_type) | childcare_type == "", NA_character_, childcare_type)
      out <- ifelse(is.na(base_val) & !is.na(other_text), other_text, base_val)
      both <- !is.na(base_val) & !is.na(other_text)
      out[both] <- paste(base_val[both], other_text[both], sep = ", ")
      out
    },
    ...
    # --- Child language: keep as-is, no modifications needed ---
  ) %>%
  rowwise() %>%
  mutate(
  ...
  select(
    ...
    childcare_start_age, childcare_size,
    name_filloutsurvey
  ) %>%
  rename(
    child_birthdate   = birthdate_1,
    child_age_in_months = age_in_months_1,
    survey_respondent = name_filloutsurvey
  ) %>%
  rename(
  ...
  mutate(
    ...
    # --- Clean childcare_type to new canonical labels ---
    # Canonicals: "Daycare center", "Family daycare", "Relative cares",
    #             "Child care provider", "Other"
    childcare_type = {
      s <- childcare_type
      # Remove any parenthetical examples and embedded Qualtrics tokens
      ...
      vapply(parts, function(v) {
        ...
        # Normalize each token to one of the five canonical labels
        v_norm <- vapply(v, function(tok) {
          t <- trimws(tok)
          t <- sub("^Other, \\(?please specify\\)?:?\\s*", "Other", t, ignore.case = TRUE, perl = TRUE)
          t <- gsub("'s own home$", "", t)
          if (grepl("^daycare\\s*center$", t, ignore.case = TRUE)) {
            "Daycare center"
          } else if (grepl("^family\\s*daycare$", t, ignore.case = TRUE)) {
            "Family daycare"
          } else if (grepl("^relative", t, ignore.case = TRUE)) {
            "Relative cares"
          } else if (grepl("^child\\s*care\\s*provider", t, ignore.case = TRUE)) {
            "Child care provider"
          } else if (grepl("^other$", t, ignore.case = TRUE)) {
            "Other"
          } else {
            t
          }
        ...
        seen <- character(0)
        for (t in v_norm) if (!(t %in% seen)) seen <- c(seen, t)
        paste(seen, collapse = ", ")
      }, character(1))
    },

    # --- Child language: keep as-is, no categorization needed ---
  )
...
if (FALSE) {
  write.csv(ego_level_network_summary, file = "ego_level_network_summary.csv", row.names = FALSE)
  write.csv(node_level_long, file = "node_level_long.csv", row.names = FALSE)
}
```
